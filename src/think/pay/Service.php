...
class Service
{
        ...
        if ($type == 'alipay') {
            ...
            if ($method == 'web') {
                ...
                if ($isWechat || $config['scanpay']) {
                    ...
                    $url = (string) url('epay/api/alipay', [], true, true);
            ...
            if ($method == 'web') {
                ...
                if ($request->isMobile() && !$isWechat) {
                    ...
                    $url = (string) url('epay/api/wechat', [], true, true);
                    return RedirectResponse::create($url);
                }
            }
    ...
    {
        $config = Config::get('pay.' . $type);
        if ($config['log']) {
            $config['log'] = [
                'file' => app()->getRuntimePath() . 'epaylogs' . DIRECTORY_SEPARATOR . $type . '-' . date("Y-m-d") . '.log',
                'level' => 'debug',
            ];
        }
        if (isset($config['cert_client']) && substr($config['cert_client'], 0, 8) == '/addons/') {
            $config['cert_client'] = root_path() . str_replace('/', DIRECTORY_SEPARATOR, substr($config['cert_client'], 1));
        }
        if (isset($config['cert_key']) && substr($config['cert_key'], 0, 8) == '/addons/') {
            $config['cert_key'] = root_path() . str_replace('/', DIRECTORY_SEPARATOR, substr($config['cert_key'], 1));
        }
        if (isset($config['app_cert_public_key']) && substr($config['app_cert_public_key'], 0, 8) == '/addons/') {
            $config['app_cert_public_key'] = root_path() . str_replace('/', DIRECTORY_SEPARATOR, substr($config['app_cert_public_key'], 1));
        }
        if (isset($config['alipay_root_cert']) && substr($config['alipay_root_cert'], 0, 8) == '/addons/') {
            $config['alipay_root_cert'] = root_path() . str_replace('/', DIRECTORY_SEPARATOR, substr($config['alipay_root_cert'], 1));
        }
        if (isset($config['ali_public_key']) && (Str::endsWith($config['ali_public_key'], '.crt') || Str::endsWith($config['ali_public_key'], '.pem'))) {
            $config['ali_public_key'] = root_path() . str_replace('/', DIRECTORY_SEPARATOR, substr($config['ali_public_key'], 1));
        }
        // 可选
        $config['http'] = [
            'timeout' => 10,
        ...
        $config['notify_url'] = empty($config['notify_url']) ? (string) url('epay/api/notifyx', [], false) . '/type/' . $type : $config['notify_url'];
        $config['notify_url'] = !preg_match("/^(http:\/\/|https:\/\/)/i", $config['notify_url']) ? request()->root(true) . $config['notify_url'] : $config['notify_url'];
        $config['return_url'] = empty($config['return_url']) ? (string) url('epay/api/returnx', [], false) . '/type/' . $type : $config['return_url'];
